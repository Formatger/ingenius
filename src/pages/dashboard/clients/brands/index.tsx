...
import { getCampaigns, getCreators, getProjectStages, getBrands, getBrandsDetail } from "@/utils/httpCalls";
...
const BrandsPage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const BrandsFullData = provisionalBrandsData.map(item => {
        const detail = provisionalBrandsDetailData.find(detail => detail.id === item.id);
        return {
          ...item,
          ...detail
        };
  ...
  const sortBy = (field: keyof typeof brandsData[0]) => {
    ...
    if (field === "total_brands" || field === "contract_value") {
      ...
      brandsDataCopy.sort((a, b) => (a[field] as string)?.localeCompare(b[field] as string));
      if (JSON.stringify(brandsDataCopy) === JSON.stringify(brandsData)) {
        // Already sorted in ascending order, so sort in descending order
        brandsDataCopy.sort((a, b) => (b[field] as string)?.localeCompare(a[field] as string));
      }
    }
